// This file is a personal hell. there probably was a better way to do this than having builtins
// as a bunch of C function registered at startup (on the bright-side the functions are probably quicker
// than any alternative because they are compiled for the hardware), it might have been better to take
// the gforth approach and have a bunch of .fs files loadded on startup or something instead of this mess
    ...
    if(state->state_var == 0){
        ...
        if(((size_t)(p - tmp) == len) && (p != tmp)){
            ...
            if(word)
                ...
                fprintf(stderr, "\x1b[1;93m[ERROR 0x%04X]:\x1b[0m\tUnidentified word: \" %.*s \"\n", 0x1019, (int)len, state->curr->start);
        }
        return;
    }
...
void forth_begin(void *na){ // BEGIN
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("BEGIN");
        return;
    }
...
void forth_until(void *na){ // UNTIL  ( flag -- )  branches back to BEGIN if false
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("UNTIL");
        return;
    }
...
void forth_again(void *na){ // AGAIN  ( -- )  unconditional loop back to BEGIN
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("AGAIN");
        return;
    }
...
void forth_while(void *na){ // WHILE  ( flag -- )
    ...
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("WHILE");
        return;
    }
...
void forth_repeat(void *na){ // REPEAT
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("REPEAT");
        return;
    }
...
void forth_recurse(void *na){ // RECURSE  compile a call to the word being defined
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("RECURSE");
        return;
    }
...
void forth_paren(void *na){         // (  inline comment  skip until )
    ...
    while(state->curr != NULL){
        size_t len = (size_t)(state->curr->end - state->curr->start);
        if(len > 0 && state->curr->start[len-1] == ')')
            return;
...
void forth_star_slash_mod(void *na){ // */MOD  ( n1 n2 n3 -- n4 n5 )
    ...
    if(n3 == 0){
        fprintf(stderr, "[ERROR]: */MOD divide by zero\n");
        return;
    }
...
void forth_accept(void *na){    // ACCEPT  ( addr len -- actual )
    ...
    if(fgets(buf, (int)maxlen + 1, stdin) == NULL){
        dPush(0);
        return;
    }
...
void forth_literal(void *na){   // LITERAL  ( n -- )  compile n as literal
    State *state = (State *)na;
    if(!state->compileMode){    // in interpret mode it's a no-op
        return;
    }
...
void forth_postpone(void *na){  // POSTPONE  compile compilation semantics of next word
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("POSTPONE");
        return;
    }
    ...
    Entry *word = dictionaryLookup(state->dict, state->curr->start, len);
    if(!word){
        fprintf(stderr, "[ERROR]: POSTPONE unknown word: %.*s\n", (int)len, state->curr->start);
        return;
    }
...
void forth_create(void *na){    // CREATE name  ( -- )
    State *state = (State *)na;
    if(state->curr->next == NULL){
        fprintf(stderr, "[ERROR]: CREATE missing name\n");
        return;
    }
    state->curr = state->curr->next;
    size_t len = (size_t)(state->curr->end - state->curr->start);
...
void forth_endof(void *na){     // ENDOF
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("ENDOF");
        return;
    }
...
void forth_endcase(void *na){   // ENDCASE  ( x -- )
    State *state = (State *)na;
    if(!state->compileMode){
        compile_error_not_in_def("ENDCASE");
        return;
    }
...
void forth_value(void *na){     // VALUE name  ( n -- )
    ...
    cell val = dPop();
    if(state->curr->next == NULL){
        fprintf(stderr, "[ERROR]: VALUE missing name\n");
        return;
    }
    state->curr = state->curr->next;
    size_t len = (size_t)(state->curr->end - state->curr->start);
...
void forth_to(void *na){        // TO name  ( n -- )
    State *state = (State *)na;
    if(state->curr->next == NULL){
        fprintf(stderr, "[ERROR]: TO missing name\n");
        return;
    }
    ...
    if(!e || !e->data_field){
        fprintf(stderr, "[ERROR]: TO: not a VALUE\n");
        return;
    }
    *e->data_field = dPop();
}
...
void forth_environment_q(void *na){ // ENVIRONMENT?  ( addr len -- false | ... true )
    ...
    if(strcmp(name, "/COUNTED-STRING") == 0){
        dPush(255); dPush((cell)-1);
        return;
    }
    if(strcmp(name, "/HOLD") == 0){
        dPush(64); dPush((cell)-1);
        return;
    }
    ...
    if(strcmp(name, "ADDRESS-UNIT-BITS") == 0){
        dPush(8); dPush((cell)-1);
        return;
    }
    if(strcmp(name, "FLOORED") == 0){
        dPush(0); dPush((cell)-1);
        return;
    }
    if(strcmp(name, "MAX-CHAR") == 0){
        dPush(255); dPush((cell)-1); return;
    }
    if(strcmp(name, "MAX-N") == 0){
        dPush((cell)((cell)1<<(sizeof(cell)*8-1))-1); dPush((cell)-1);
        return;
    }
    if(strcmp(name, "MAX-U") == 0){
        dPush((cell)-1); dPush((cell)-1);
        return;
    }
